          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderTop: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <button disabled={filters.page === 1} onClick={() => setFilters(f => ({ ...f, page: f.page - 1 }))}
                style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '5px 12px', fontSize: 12, color: 'var(--text-primary)', cursor: filters.page === 1 ? 'not-allowed' : 'pointer', opacity: filters.page === 1 ? 0.5 : 1 }}>
                ← Prev
              </button>
              <button disabled={filters.page === data.pages} onClick={() => setFilters(f => ({ ...f, page: f.page + 1 }))}
                style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '5px 12px', fontSize: 12, color: 'var(--text-primary)', cursor: filters.page === data.pages ? 'not-allowed' : 'pointer', opacity: filters.page === data.pages ? 0.5 : 1 }}>
                Next →
              </button>
            </div>
          </div>
